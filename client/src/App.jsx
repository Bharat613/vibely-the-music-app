...
function AppContent() {
  ...
  const [currentViewingList, setCurrentViewingList] = useState(null);
  ...
  const famousMusicDirectors = [
    { name: 'Devi Sri Prasad', query: 'Devi Sri Prasad songs', image: '/images/dsp-art.jpg' },
    { name: 'Thaman S', query: 'Thaman S songs', image: '/images/thaman-art.jpg' },
    { name: 'Anirudh Ravichander', query: 'Anirudh Ravichander songs', image: '/images/anirudh-art2.jpg' },
    {name: "M. M. Keeravani",query: "M. M. Keeravani songs", image: "/images/keeravani-art.jpg"},
  ];
...
useEffect(() => {
  ...
  const handleSingerClick = async (artist) => {
    // toast.info(`Fetching songs by ${artist.name}...`);
    try {
      const res = await fetch(`${SAAVN_API_URL}/search/songs?query=${encodeURIComponent(artist.query)}&bitrate=320&limit=50`);
      const data = await res.json();
      if (data.success && data.data?.results?.length > 0) {
        const songs = data.data.results.map((song) => {
          const cleanedTitle = song.name.replace(/\s*\(.*?\)|\[.*?\]/g, "").trim();
        ...
        const artistList = { name: `${artist.name}'s Hits`, songs: songs };
        setCurrentViewingList(artistList);
        navigate("/view-list");
      } else {
        toast.error(`❌ Songs by ${artist.name} not found!`);
      }
    } catch (err) {
      console.error(err);
      toast.error("Failed to fetch songs. Please try again.");
          ...
          {s.title}
                ...
                <div className="main-content">
                  <Home
                    ...
                    famousMusicDirectors={famousMusicDirectors}
                    onDirectorClick={handleSingerClick}
                  />
                </div>
              </>
            ) : (
