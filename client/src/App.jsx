...
      const success = await addSongToPlaylist(playlistName, songList[currentSongIndex]);
      if (success) {
...
  // --- MODIFIED LOGIC ---
  // Now, this function automatically adds the song after creating the playlist.
...
      const data = await response.json();
      if (data.success) {
        // Refresh the playlist list to show the new one.
        await fetchPlaylists(currentUser.email);
        
        // Automatically add the current song to the newly created playlist.
        const songAddedSuccess = await addSongToPlaylist(newPlaylistName, songList[currentSongIndex]);
        
        if (songAddedSuccess) {
//           toast.success(`Playlist "${newPlaylistName}" created and song added!`);
toast.success("song added!");
          setShowPlaylistSelector(false); // Close the selector once done.
          return true;
        } else {
          // Handle the case where playlist creation succeeded but song addition failed.
